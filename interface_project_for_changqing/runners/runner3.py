...
def runcase_new(testsuit,cookies):
    ...
    for i in range(len(testsuit)):
        ...
        logger.info("执行用例id=%s,name=%s",caseinfo['id'],caseinfo['name'] )
        #print(caseinfo['url'])
        if caseinfo['flag'] == 'gety':
            result = method.gy(caseinfo, headers, cookies)
        else:
            result = method.py(caseinfo, headers, cookies)
        logger.info("执行用例[id=%s,name=%s]执行结果:%s", caseinfo['id'], caseinfo['name'],result[0])
