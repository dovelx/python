...
workticketmbcdid = 'SELECT workticketmbcdid FROM `hap_hse_clsh`.`hse_ticket_mbcd` ORDER BY `workticketmbcdid` DESC LIMIT 1'
...
sql_query_jsastepid = 'SELECT jsastepid FROM `hap_hse_clsh`.`hse_safety_analysis_harm` ORDER BY `jsa_step_harm_id` DESC LIMIT 1;'
