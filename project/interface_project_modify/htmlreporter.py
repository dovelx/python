    ...
    def generate_html(self, file):
        ...
        for row in result:
            ...
            tab1 << tr(td('ID', bgcolor='#ABABAB', align='center')
                           ...
                           + td('用例外部ID', bgcolor='#ABABAB', align='center')
            ...
            for row in result:
                ...
                tab1 << tr(td(str(row[0]), align='center') + td(row[1], align='center') + td(row[2], align='center') +
                                td('<a name=\"first'+str(row[3]) + project + testplan + '\"' + 'href=\"#second' + str(row[3]) + project + testplan + '\">' + row[3] + '</a>')
                                + td(row[4], align='left') + td(row[5], align='left') + td6
        ...
        for row in result:
            ...
            query = ('SELECT tc_external_id, testcase_name, project, testplan, case_exec_history_id, testcase_id FROM ' + testcase_report_tb + \
            ...
            for row in result:
                ...
                page << p('>>>测试用例【用例外部ID：' + tc_external_id + '，名称：<a name=\"second'+ tc_external_id + project + testplan +'\"' + \
                          'href=\"#first' + tc_external_id + project + testplan + '\">' + case_name + '</a>】')

                tab = table( border='1', cellpadding='1', cellspacing='0', cl='table')
                tab2 = page << tab
                tab2 << tr(td('步序', bgcolor='#ABABAB', align='center')
                ...
                for row in result:
                    ...
                    tab2 << tr(td(str(row[0]), align='center') + td(row[1], align='center') + td(row[2], align='center') +
                            td(row[3], align='left') +  td(row[4], align='center') + td('<xmp>' + str(row[5]) + '</xmp>', align='left') +
                            td(str(row[6]), align='left') + td7 + td(row[8], align='left') +  td(row[9], align='center'))
                page << p('<br/>')
