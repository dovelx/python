...
def get_test_case_reports(report_id, suite_id):
    try:
        ...
        for report in reports:
            if "resultDetail" in report and "responseData" in report['resultDetail']:
                del report['resultDetail']['responseData']
        return jsonify({'status': 'ok', 'data': {'totalNum': total_num, 'rows': reports}})
    except BaseException as e:
        current_app.logger.error("get report detail failed. - %s" % str(e))
        return jsonify({'status': 'failed', 'data': 'get report detail failed %s' % str(e)})


@app.route('/api/testReportDetail/<report_id>/detail/<detail_id>', methods=['GET'])
@login_required
def get_test_report_detail(report_id, detail_id):
    res = TestReportDetail.find_one({'_id': ObjectId(detail_id), 'reportId': ObjectId(report_id)})
    return jsonify({'status': 'ok', 'data': common.format_response_in_dic(res)}) if res else \
        jsonify({'status': 'failed', 'data': '未找到该report信息'})
