...
def login():
    ...
    import json
    #访问1-主页
    url = 'http://hdxk.ushayden.com/'

    #访问主页的header
    headers = {
        "Host": "hdxk.ushayden.com",
        ...
        "Cookie": "tenantCode=hd; prjidentify=hd; module=portals"
    }
   ...
   #访问2-302的header
    ...
    session = requests.Session()
    #url="http://hdxk.ushayden.com/login?service=http%3A%2F%2Fhdxk.ushayden.com%2Fcas&tenantCode=hd&trial=false"
    url = location_url
    print("url-2",url)
    ...
    b = {}
    b['Cookie'] = "TENANTCODE=hd; tenantCode=hd; prjidentify=hd; module=passports; JSESSIONID=" + first_cookie
    #print("2-b",b)
    #print(res.content)
    #获取验证it，为拼写密码串使用
    ...
    headers={
		...
		"Referer": "http://hdxk.ushayden.com/login?service=http%3A%2F%2Fhdxk.ushayden.com%2Fcas&tenantCode=hd&trial=false",
        "Accept-Encoding": "gzip, deflate",
        "Accept-Language": "zh-CN,zh;q=0.9,en;q=0.8,zh-TW;q=0.7",
        "Cookie": "TENANTCODE=hd; module=passports; JSESSIONID=3C40E9ADD7EB762495CD37882AB1E60F"
    }
    headers['Cookie'] = b['Cookie']
    url = "http://hdxk.ushayden.com/themes/hayden/images/login/captcha.png"
    res = session.get(url=url, headers=headers, allow_redirects=False)
    #访问3-登录验证地址
    url = "http://hdxk.ushayden.com/passports/login"
    res = session.get(url=url, headers=headers, allow_redirects=False)
    #print(res.headers)
    #print("3-",res.content)
    # 访问4-获取公key
    url = 'http://hdxk.ushayden.com/preLogin/getPubKey'
    res = session.get(url=url, headers=headers, allow_redirects=False)
    ...
    headers = {
        ...
        "Upgrade-Insecure-Requests": "1",
        "Origin": "http://hdxk.ushayden.com",
        "Content-Type": "application/x-www-form-urlencoded",
        "User-Agent": "Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.129 Safari/537.36",
        "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
        "Referer": "http://hdxk.ushayden.com/passports/login?service=http%3A%2F%2Fhdxk.ushayden.com%3A80%2Fportals%2Fcas&tenantCode=hd&trial=false",
        "Accept-Encoding": "gzip, deflate",
        "Accept-Language": "zh-CN,zh;q=0.9,en;q=0.8,zh-TW;q=0.7",
        "Cookie": "TENANTCODE=hd;tenantCode=hd; prjidentify=hd; module=passports; JSESSIONID=3C40E9ADD7EB762495CD37882AB1E60F"
    }
    ...
    #拼接请求密码数据
    data = "username=admin2022&verifyPwd=" + pwd + "&captcha=&rememberMe=false&lt=" + it + "&execution=" + execution + "&_eventId=submit&module=pub-base&password=" + pwd
    ...
    headers = {
        ...
        "User-Agent": "Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.129 Safari/537.36",
        "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
        "Referer": "http://hdxk.ushayden.com/",
        "Accept-Encoding": "gzip, deflate",
        "Accept-Language": "zh-CN,zh;q=0.9,en;q=0.8,zh-TW;q=0.7",
        "Cookie": "TENANTCODE=hd;tenantCode=hd; module=passports; prjidentify=hd; JSESSIONID=BE35E2D7C037E16DD7C0801A75F3FBE1;CASPRIVACY=;TGC=eyJhbG"
    }
    ...
    url= 'http://hdxk.ushayden.com/portals/portals/hd'

    rs = requests.get(url=url, headers=headers)
    data = rs.content.decode('utf-8')

    url= 'http://hdxk.ushayden.com/portals/hd'
