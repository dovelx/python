...
#SELECT * FROM `hap_hse`.`hse_ticket_mbcd` ORDER BY `workticketmbcdid` DESC LIMIT 0,1000

workticketmbcdid = 'SELECT workticketmbcdid FROM `hap_hse`.`hse_ticket_mbcd` ORDER BY `workticketmbcdid` DESC LIMIT 1'
...
sql_query_jsa_step_harm_id = 'SELECT jsa_step_harm_id FROM `hap_hse`.`hse_safety_analysis_harm` ORDER BY `jsa_step_harm_id` DESC LIMIT 1;'

sql_query_jsastepid = 'SELECT jsastepid FROM `hap_hse`.`hse_safety_analysis_harm` ORDER BY `jsa_step_harm_id` DESC LIMIT 1;'

sql_query_jsa_step_measure_id = 'SELECT jsa_step_measure_id FROM `hap_hse`.`hse_safety_analysis_measure` ORDER BY `jsa_step_measure_id` DESC LIMIT 1'

#SELECT worknumber FROM `hap_hse`.`hse_safety_task` ORDER BY `worktaskid` DESC LIMIT 1

sql_query_jsa_worknumber = 'SELECT worknumber FROM `hap_hse`.`hse_safety_task` ORDER BY `worktaskid` DESC LIMIT 1'

sql_query_jsa_workname= 'SELECT workname FROM `hap_hse`.`hse_safety_task` ORDER BY `worktaskid` DESC LIMIT 1'
